      ...
      body: Padding(
        ...
        child: loading ? Center(child: CircularProgressIndicator()) : Column(
          children: [
            Row(
              children: [
                Expanded(child: TextField(decoration: InputDecoration(prefixIcon: Icon(Icons.search), hintText: 'Buscar por ID ou título...'))),
                SizedBox(width: 12),
                ElevatedButton.icon(onPressed: () {}, icon: Icon(Icons.add), label: Text('Novo Chamado'))
              ],
            ),
            SizedBox(height: 12),
            Expanded(
              child: ListView.separated(
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return ListTile(
                    ...
                    trailing: Text(c.status, style: TextStyle(color: c.status=='Aberto' ? Colors.red : Colors.green)),
                    onTap: () {},
                  );
                },
              ),
