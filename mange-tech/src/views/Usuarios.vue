  ...
  <div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6">
      <div class="relative">
        ...
        <input v-model="searchTerm" type="text" placeholder="Buscar por nome, email ou username..."
    ...
    <div v-else-if="error"
      class="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg flex justify-between items-center">
      <span>{{ error }}</span>
    ...
    <div v-else>
      ...
      <div class="hidden lg:block bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="perfil in usuariosFiltrados" :key="perfil.user?.id || perfil.id"
              @click="irParaDetalhes(perfil.id)" class="hover:bg-indigo-50 transition-colors group cursor-pointer">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-10 w-10">
                    <div
                      class="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold border border-indigo-200 text-sm">
                      {{ getIniciais(perfil) }}
                    </div>
                  </div>
                  <div class="ml-4">
                    ...
                    <div class="text-[10px] text-gray-300">ID: {{ perfil.id }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button @click.stop="abrirModalEditar(perfil)"
                  class="text-blue-400 hover:text-blue-600 mr-3 p-1 rounded hover:bg-blue-50 transition" title="Editar">
                  <Edit2 :size="18" />
                </button>
                <button @click.stop="confirmarRemocao(perfil)"
                  class="text-red-400 hover:text-red-600 p-1 rounded hover:bg-red-50 transition" title="Remover">
                  <Trash2 :size="18" />
                </button>
              </td>
            </tr>
      ...
      <div class="lg:hidden space-y-4">
        <div v-for="perfil in usuariosFiltrados" :key="perfil.user?.id || perfil.id" @click="irParaDetalhes(perfil.id)"
          ...
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-[10px] text-gray-300">ID: {{ perfil.id }}</p>
              </div>
            </div>
          ...
          <div class="space-y-2 text-sm text-gray-600 mb-4 border-t border-gray-100 pt-3">
            <div class="flex justify-between"><span class="text-gray-400">Email:</span> <span
                class="font-medium text-gray-800">{{ perfil.user?.email || '-' }}</span></div>
            <div class="flex justify-between"><span class="text-gray-400">Telefone:</span> <span
                class="font-medium text-gray-800">{{ perfil.telefone || '-' }}</span></div>
          </div>

          <div class="flex gap-2">
            <button @click.stop="abrirModalEditar(perfil)"
              class="flex-1 py-2 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold uppercase tracking-wide hover:bg-blue-100 transition">Editar</button>
            <button @click.stop="confirmarRemocao(perfil)"
              class="flex-1 py-2 bg-red-50 text-red-600 rounded-lg text-xs font-bold uppercase tracking-wide hover:bg-red-100 transition">Remover</button>
          </div>
        </div>
    ...
    <div v-if="mostrarModal"
      ...
      <div class="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl animate-scale-in">
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-100 sticky top-0 bg-white z-10">
          ...
          <button @click="fecharModal" class="text-gray-400 hover:text-gray-600 bg-gray-50 p-2 rounded-full transition">
            <X :size="20" />
          </button>
        </div>
        ...
        <form @submit.prevent="salvarUsuario" class="p-6 space-y-5">
          <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">Username <span
                    class="text-red-500">*</span></label>
                <input v-model="formUsuario.username" type="text" required :disabled="editando"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white disabled:bg-gray-100 disabled:text-gray-500"
                  placeholder="joaosilva" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">Email <span
                    class="text-red-500">*</span></label>
                <input v-model="formUsuario.email" type="email" required :disabled="editando"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white disabled:bg-gray-100 disabled:text-gray-500"
            ...
            <div v-if="!editando" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">Senha <span
                    class="text-red-500">*</span></label>
                <input v-model="formUsuario.password" type="password" required minlength="6"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                  placeholder="******" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">Confirmar Senha <span
                    class="text-red-500">*</span></label>
                <input v-model="formUsuario.password_confirm" type="password" required
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                  placeholder="******" />
              </div>
            </div>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">Nome <span
                    class="text-red-500">*</span></label>
                <input v-model="formUsuario.first_name" type="text" required
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  placeholder="João" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">Sobrenome <span
                    class="text-red-500">*</span></label>
                <input v-model="formUsuario.last_name" type="text" required
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  placeholder="Silva" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">Telefone</label>
                <input v-model="formUsuario.telefone" type="tel"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  placeholder="(11) 99999-9999" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">NIF / CPF</label>
                <input v-model="formUsuario.nif" type="text"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  placeholder="000.000.000-00" />
              </div>
              <div class="sm:col-span-2">
                <label class="block text-xs font-semibold text-gray-700 mb-1.5">Endereço Completo</label>
                <input v-model="formUsuario.endereco" type="text"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
          ...
          <div v-if="sucessoForm"
            class="p-3 bg-green-50 border border-green-200 text-green-700 rounded-lg text-sm flex items-center gap-2">
            ✅ {{ sucessoForm }}
          </div>

          <div class="flex flex-col sm:flex-row gap-3 justify-end pt-4 border-t border-gray-100">
            <button type="button" @click="fecharModal"
              class="px-5 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium transition order-2 sm:order-1"
              :disabled="salvando">Cancelar</button>
            <button type="submit"
              class="px-5 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium shadow-md transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 order-1 sm:order-2"
              :disabled="salvando">
              <span v-if="salvando"
      ...
      <div class="bg-white rounded-xl w-full max-w-sm p-6 shadow-2xl animate-scale-in">
        <div class="text-center mb-6">
          ...
          <p class="text-sm text-gray-500">
            Você está prestes a remover o usuário <strong>{{ usuarioParaRemover?.user?.username }}</strong>. Esta ação
            não pode ser desfeita.
          </p>
        </div>
        <div class="flex gap-3 justify-center">
          <button @click="fecharModalRemocao"
            class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium transition flex-1"
            :disabled="removendo">Cancelar</button>
          <button @click="removerUsuario"
            class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium shadow-md transition flex-1 flex items-center justify-center gap-2"
            :disabled="removendo">
            <span v-if="removendo"
              class="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full"></span>
...
const buscarUsuarios = async () => {
  ...
  try {
    ...
    if (usuarios.value.length > 0) {
      console.log('🔎 Exemplo de objeto usuário:', usuarios.value[0])
      if (!usuarios.value[0].id) console.warn('⚠️ ALERTA: O objeto usuário NÃO tem a propriedade "id" na raiz!')
    }
...
const irParaDetalhes = (id) => {
  console.log('🖱️ Clique detectado na linha! ID recebido:', id)

  if (!id) {
    alert('Erro: ID do usuário está vazio ou indefinido.')
    console.error('❌ ID inválido. Verifique se o objeto "perfil" tem a propriedade "id".')
    return
  }

  console.log('🚀 Navegando para rota "usuario-detalhes" com ID:', id)
